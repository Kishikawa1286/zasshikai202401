...
- 古典的な AHP では意思決定者は代替案の優先順位を求めるために一対比較を行う
  - この結果として得られる crisp な一対比較値は乗法的比較行列 (Multiplicative comparison matrix, MCM) として表される (Saaty, 1980)

- 意思決定者の一対比較のあいまいさを表すために用いられる
  - 乗法的区間比較行列 (Interval MCM, IMCM) (Saaty and Vargas, 1987)
  - 加法的区間比較行列 (Interval additive comparison matrices) (Xu and Chen, 2008)
...
- Wang and Elhag, 2007
  - 各成分が IMCM $\bar{A} = ([\bar{a}_{ij}^\mathrm{L}, \bar{a}_{ij}^\mathrm{U}])$ に含まれる, 整合している crisp MCM $A = (a_{ij})$ が存在すれば IMCM $\bar{A}$ は整合している
  $a_{ij} \in [\bar{a}_{ij}^\mathrm{L}, \bar{a}_{ij}^\mathrm{U}], ~~ i,j = 1,2,\dots,n$

- Wang, 2015a
  - IMCM $\bar{A} = (\bar{a}_{ij}) = ([\bar{a}_{ij}^\mathrm{L}, \bar{a}_{ij}^\mathrm{U}])$ が次の条件を満たすならばIMCM $\bar{A}$ は整合している
...
- Wang, Yang and Xu, 2005b
  - IMCM $\bar{A} = ([\bar{a}_{ij}^\mathrm{L}, \bar{a}_{ij}^\mathrm{U}])$ に対して, 次の制約を満たす crisp なベクトル $w = (w_i)$ が存在するならば IMCM $\bar{A}$ は整合している
  $$
  \begin{align*}
...
- Wang and Elhag, 2007
    ...
    $$
    しかし, 一般に意思決定者が与える IMCM $A = ([a_{ij}^\mathrm{L}, a_{ij}^\mathrm{U}])$ はこれを満たさない.
    そこで, 偏差ベクトル $\Epsilon = (\epsilon_i), \Gamma = (\gamma_i)$ を次のように定める.
    $$
...
- Wang（2015a）のIMCMsの整合性定義を使用する
- 整合した IMCM の性質について調べる
- 許容可能な不確定性比率 (indeterminacy ratio) の閾値を導入
  - 整合していてもあいまいな場合は許容しない
- IMCMの不確定性の指標を定める
- 許容可能な IMCM から重みを求める手法の提案
  - 二次計画問題
...
- $\bar{A} = (\bar{a}_{ij}) = ([ \bar{a}^-_{ij}, \bar{a}^+_{ij}])$: 区間一対比較行列
  $$
...
次の一対比較から, 代替案の並びが異なる二つの区間一対比較行列 $\bar{A}_1, \bar{A}_1^\prime$ を考える.
...
$\bar{A} = (\bar{a}_{ij}) = ([ a^-_{ij}, a^+_{ij}])$ を IMCM とする.
...
Example 1 の二つの IMCM $\bar{A}_1, \bar{A}_1^\prime$ はともに Definition 4.1 の意味で整合していない.
...
$\bar{A} = (\bar{a}_{ij}) = ([ a^-_{ij}, a^+_{ij}])$ を $1/S \leq a_{ij}^- \leq a_{ij}^+ \leq S$ を満たす IMCM とする.
...
意思決定者がすべての一対比較値が $[1/S, S]$ である IMCM を与えた場合, Definition 4.1 の意味で整合している.
しかし, これは意思決定者が比較について全く確信していないことを意味し, この IMCM から良い意思決定結果を得ることはできない.
このような場合, IMCM の大きすぎる不確定性のために受け入れられないとする必要がある.
...
$\bar{A} = (\bar{a}_{ij}) = ([ a^-_{ij}, a^+_{ij}])$ を IMCM とし, $A^\mathrm{gm} = \left( \sqrt{a_{ij}^- a_{ij}^+} \right)$ とする.
$t_\mathrm{ur} ( \geq 1)$ を IMCM の許容可能な不確定性比率 (IR) のしきい値とする.
...
$\bar{A} = (\bar{a}_{ij}) = ([ a^-_{ij}, a^+_{ij}]), ~ \bar{B} = (\bar{b}_{ij}) = ([ b^-_{ij}, b^+_{ij}])$ を IMCM とする.
...
$\bar{A}^{(l)} = \left( \bar{a}_{ij}^{(l)} \right) = \left( \left[ a_{ij}^{-(l)}, a_{ij}^{+(l)} \right] \right), ~ (l = 1,2,\dots,m)$ を $m$ 人の意思決定者が与えた IMCM とする.
$\alpha_l \geq 0, l = 1,2,\dots,m$ を意思決定者の重みとし, $\sum_{l=1}^m \alpha_l = 1$ とする.
$\bar{A}^{(l)}, l = 1,2,\dots,m$ を次のように結合して, グループの IMCM $\bar{A}^\mathrm{G} = \left( \bar{a}_{ij}^\mathrm{G} \right) = \left( \left[ a_{ij}^{-\mathrm{G}}, a_{ij}^{+\mathrm{G}} \right] \right)$ を得る.
...
$\bar{A}^{(l)} = \left( \bar{a}_{ij}^{(l)} \right) = \left( \left[ a_{ij}^{-(l)}, a_{ij}^{+(l)} \right] \right), ~ (l = 1,2,\dots,m)$ を $m$ 人の意思決定者が与えた IMCM とし, $\bar{A}^\mathrm{G} = \left( \bar{a}_{ij}^\mathrm{G} \right) = \left( \left[ a_{ij}^{-\mathrm{G}}, a_{ij}^{+\mathrm{G}} \right] \right) = \left( \left[ \prod_{l = 1}^m \left( a_{ij}^{-(l)} \right)^{\alpha_l} , \prod_{l = 1}^m \left( a_{ij}^{+(l)} \right)^{\alpha_l} \right] \right)$ をグループの IMCM とする.
$\alpha_l \geq 0, l = 1,2,\dots,m$ を意思決定者の重みとし, $\sum_{l=1}^m \alpha_l = 1$ とする.
